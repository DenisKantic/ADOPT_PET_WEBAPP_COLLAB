...
import { getLostPetPostDashboard } from '@public/actions/getLostPetPost'
...
export default function LostPetPost() {
  const [animalPost, setAnimalPost] = useState<LostPetItem[]>([])
  ...
  const fetchPost = async (email: any) => {
    try {
      ...
      const response = await getLostPetPostDashboard({ email })
      const processedPost: LostPetItem[] = response.lost_post.map(
        (postItem) => {
          const imagePaths =
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 bg-black px-20 gap-5 bg-opacity-70 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <Link
                onClick={() => console.log('SLUG ONE IITEM', item.slug)}
                href={`/dashboard/lostPetDetails/${item.slug}`}

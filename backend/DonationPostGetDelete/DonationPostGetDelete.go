...
func GetAllDonationPosts(w http.ResponseWriter, r *http.Request) {
	...
	// Construct the base query to get the 5 newest posts
	query := `
		SELECT id,image_paths, post_category, animal_category, post_name, phone_number, description, location, slug, user_email, created_at 
		FROM donationPost
	...
	// Execute the query to get the 5 most recent posts
	...
	for rows.Next() {
		...
		err = rows.Scan(&id, &image_paths, &postCategory, &animalCategory, &postName, &phoneNumber, &description, &location, &slug, &userEmail, &createdAt)
		if err != nil {
		...
		post := map[string]interface{}{
			"id":              id,
			"image_paths":     image_paths, // Assuming image_paths is a comma-separated string
